...
def draw_map(results, lat, lon, service):
    strTemp1 = """ <form action="get-method.php" method="get"> <input type="text" name="ip" placeholder="IP" value=" """
    strTemp2 = """ " /><input type="submit" name="submit" /> </form> """
    repeats = []
    coordinates = []
    ...
    if service == "printer":
        ...
        for counter, printer in enumerate(results['matches']):
            ...
            popup_text = ip + "<br>" + product + "<br><button>Scan with Nmap</button> <button>Scan CMS</button> <br>" +strTemp1 +ip +strTemp2
    ...
    if service == 'camera':
        ...
        for counter, camera in enumerate(results['matches']):
            ...
            popup_text = ip + "<br>" + product + "<br>" + str(distance_compare)[0:5] + "" + unit + " from target <br> <button>Scan with Nmap</button> <button>Scan CMS</button> <br>" +strTemp1 +ip +strTemp2
