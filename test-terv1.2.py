...
@app.route('/a/shell/<ip>')
def shell(ip):
    sip = str(ip)
    cmd = ["sh","shellshock.sh",sip]
    p = subprocess.Popen(cmd, stdout = subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            stdin=subprocess.PIPE)
    out,err = p.communicate()
...
@app.route('/a/count/<type>')
def count(type):
    stype = str(type)
    cmd = ["sh","count.sh",stype]
    p = subprocess.Popen(cmd, stdout = subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            stdin=subprocess.PIPE)
    out,err = p.communicate()
...
@app.route('/a/vncount/<type>')
def vncount(type):
    stype = str(type)
    cmd = ["sh","vncount.sh",stype]
    p = subprocess.Popen(cmd, stdout = subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            stdin=subprocess.PIPE)
    out,err = p.communicate()
